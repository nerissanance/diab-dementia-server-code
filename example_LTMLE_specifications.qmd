...
d <- readRDS(here("data/example_simulated_data.RDS"))
...
Insulin, hypertension, and 2nd line diabetes drugs not a part of the Also including death as a competing risk by including it in $L$

## Target Causal Parameter 1
...
#NOTE! This might need to be modified to match the previous Anode of the same drug with a grepl function
...
what I did is a little worse and affects more than just the treatment
holiday analysis, because I also set A_k=0 in all patients who start
with A_0=0.

your example of a dynamic rule does not yet explain how to implement
treatment holidays and censoring due to third drug. these do not seem to
be dynamic treatment rules ...

1/ Treatment holidays:

I can create a dynamic rule that allows treatment holidays, but when
should such a treatment holiday start? right now I am thinking that
changing the actually observed data still makes sense for treatment
holidays: we assume that the drug is still active in the body for up to
2 intervals. but, I could also censor patients when their treatment
holidays get too long by alterning the C_nodes. I am still confused ...

2/ Censoring due to third drug

This is not a treatment rule. I could of course artificially censor
these patients when this happens, i.e., modify the C_nodes.
