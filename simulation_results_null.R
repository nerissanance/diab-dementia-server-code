...
#Remove no Det Q
res_null_diff <- res_null_diff %>% filter(DetQ!="No")
...
res_null_diff$estimator[grepl("gbound",res_null_diff$filenames)] <- "G bound:"
...
res_null_table <- res_null_diff_coverage %>% select(filenames,iptw, estimator, Qint,bias,variance,bias_se_ratio,oracle.coverage) %>%
