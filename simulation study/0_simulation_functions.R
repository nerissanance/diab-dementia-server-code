...
run_ltmle_glmnet_old <- function(d,
                      N_time = 11, #number of time points you want to look at
                      SL.library = c("SL.glmnet"),
                      resdf=NULL,
                      Qint=F,
  ...
  if(Qint){
    ...
    if(elastic.net){
      ...
      if(useMin){
        ...
        package_stub("SuperLearner", "SuperLearner", SuperLearner_override_EN_1se, {
          testthatsomemore::package_stub("ltmle", "Estimate", Estimate_override, {
            try(res <- ltmle(data=spec_ltmle$data,
                             Anodes = spec_ltmle$Anodes,
                             Cnodes = spec_ltmle$Cnodes,
    ...
    package_stub("SuperLearner", "SuperLearner", SuperLearner_override, {
      testthatsomemore::package_stub("ltmle", "Estimate", Estimate_override, {
        ...
        package_stub("SuperLearner", "SuperLearner", SuperLearner_override_1se, {
          testthatsomemore::package_stub("ltmle", "Estimate", Estimate_override, {
            try(res <- ltmle(data=spec_ltmle$data,
                             Anodes = spec_ltmle$Anodes,
                             Cnodes = spec_ltmle$Cnodes,
