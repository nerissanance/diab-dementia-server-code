...
for(i in 1:length(d_wide_list)){
  ...
  res_df <- foreach(j = 1:200, .combine = 'bind_rows', .errorhandling = 'remove') %dopar% {
    ...
    res <- NULL
    try(res <- run_ltmle_simple(dboot,  varmethod = "ic", SL.library = c("glm"), id=dboot$id), silent=TRUE)
  ...
  gc()
  res_df$iteration <- i
  saveRDS(res_df, paste0(here::here(),"/data/sim_simple/bootstrap/sim_res_boot_glm_CV_simple_200iter_",i,".RDS"))
