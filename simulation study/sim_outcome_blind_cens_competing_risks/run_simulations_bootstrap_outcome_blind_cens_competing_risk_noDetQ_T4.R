...
#for(i in 1:length(d_wide_list)){
  ...
  res_df <- foreach(j = 1:200, .combine = 'bind_rows', .errorhandling = 'remove') %dopar% {
    ...
    res <- NULL
    try(res <- run_ltmle_glmnet(dboot, N_time = 4, resdf=NULL, Qint=FALSE, det.Q=FALSE, varmethod = "ic", id=dboot$id), silent=TRUE)
    return(res)
  }
  res_df
  ...
  saveRDS(res_df, paste0(here::here(),"/data/bootstrap/sim_res_boot_outcome_blind_cens_competing_risks_noDetQ_T4_",i,".RDS"))
...
saveRDS(resdf_boot, paste0(here::here(),"/data/sim_res_boot_outcome_blind_cens_competing_risks_noDetQ_T4.RDS"))
